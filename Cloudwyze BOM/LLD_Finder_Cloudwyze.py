...
def find_gdb_folders(start_directory):
    found_folders = []
    target_folder = "cloudwyze_lld_template_06-09-22.gdb"  # Updated target folder name
    for root, dirs, files in os.walk(start_directory):
        lowercase_root = root.lower()  # Convert root path to lowercase
        if "archive" in lowercase_root:  # Check if "archive" is in the path (case insensitive)
            continue
        
        if "asbuilt" in lowercase_root:  # Check if "asbuilt" is in the path (case insensitive)
