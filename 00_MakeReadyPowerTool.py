...
def combine_and_filter_excel_files(folder_path):
    ...
    try:
        ...
        column_set = set()  # Store unique columns across all workbooks
            ...
            if file_name.endswith('.xlsx') and not file_name.startswith('~$'):
                ...
                try:
                    ...
                    for sheet_name in wb.sheetnames:
                        ...
                        # Get column headers (assuming the headers are in the first row)
                        headers = [cell.value for cell in sheet[1]]

                        # Update the set of columns
                        column_set.update(headers)

                except Exception as e:
                    print(f"Error processing file {file_path}: {str(e)}")

        # Iterate through each workbook again to add missing columns
        for file_name in os.listdir(folder_path):
            if file_name.endswith('.xlsx') and not file_name.startswith('~$'):
                file_path = os.path.join(folder_path, file_name)
                ...
                for sheet_name in wb.sheetnames:
                    ...
                    for column in column_set:
                        if column not in headers:
                            # Find the index where the column should be inserted
                            column_index = headers.index(headers[-1]) + 1 if headers else 1
    ...
    for file_name in os.listdir(folder_path):
            if file_name.endswith('.csv') and not file_name.startswith('~$'):
                try:
                    ...
                    new_xlsx_file_path = xlsx_file_path.replace(' CSV', '_XLSX').replace('_CSV', '_XLSX')  # Remove 'XLSX' or '_XLSX' from the filename
                    os.rename(xlsx_file_path, new_xlsx_file_path)
